            ...
            if (bme680_get_results_float (sensor, &values))
                ESP_LOGI(TAG,"%.3f BME680 Sensor: %.2f °C, %.2f %%, %.2f hPa, %.2f Ohm\n",
                       ...
                       values.pressure, values.gas_resistance);
                        ...
                        /*    
                        // Publish Telemetry via MQTT Timeseries topic        
                        cJSON *root = cJSON_CreateObject();
                        cJSON_AddNumberToObject(root, "temperature", bme.temperature);
...
/* Initialisation routine for BME680 sensor - returns 'true' if BME680 found otherwise 'false' */
bool bme680_init(int addr)
{
    // Init all I2C bus interfaces at which BME680 sensors are connected
    i2c_init(I2C_BUS, I2C_SCL_PIN, I2C_SDA_PIN, I2C_FREQ);

    // init the sensor with slave address BME680_I2C_ADDRESS = 0x76 connected to I2C_BUS.
    sensor = bme680_init_sensor (I2C_BUS, BME680_I2C_ADDRESS, 0);
    if (sensor)
    {
        ...
        bme680_set_ambient_temperature (sensor, 21);
        ...
        ESP_LOGI(TAG, "BME680 sensor found and initialised");
        // Create a task that uses the sensor
        xTaskCreate(user_task, "user_task", TASK_STACK_DEPTH, NULL, 2, NULL);
        return true;
    }
    else
        ESP_LOGI(TAG, "Could not find a valid BME680 sensor, check wiring!");
        return false;
...
void app_main()
{
    isBME600 = bme680_init(BME680_I2C_ADDRESS);
